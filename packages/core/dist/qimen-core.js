...
import { DI_ZHI, GAN_WUXING, TIAN_GAN, YI_MA_MAP } from './constants/ganzhi.js';
...
// 九星原始宫位（用于伏吟/反吟判断）
const STAR_ORIGINAL_PALACE = {
    '天蓬星': 0, '天芮星': 1, '天冲星': 2, '天辅星': 3, '天禽星': 4,
    '天心星': 5, '天柱星': 6, '天任星': 7, '天英星': 8,
};
// 对冲宫位
const OPPOSITE_PALACE = { 0: 8, 8: 0, 1: 7, 7: 1, 2: 6, 6: 2, 3: 5, 5: 3 };
// 地支→宫位映射
...
function getSeason(monthBranch) {
    if (['寅', '卯'].includes(monthBranch))
        return '春';
    if (['巳', '午'].includes(monthBranch))
        return '夏';
    if (['辰', '戌', '丑', '未'].includes(monthBranch))
        return '四季土';
    if (['申', '酉'].includes(monthBranch))
        return '秋';
    return '冬';
}
...
export function calculateQimenData(input) {
