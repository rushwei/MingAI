import { traditionalYaoName } from '../../liuyao-core.js';
import { normalizeDetailLevelBinary } from '../../render-utils.js';
export function renderMeihuaCanonicalText(result, options = {}) {
    const detailLevel = normalizeDetailLevelBinary(options.detailLevel);
    const lines = ['# 梅花易数', '', '## 起卦信息'];
    ...
    if (result.castMeta.resolvedMode)
        lines.push(`- 实际子方式: ${result.castMeta.resolvedMode}`);
    if (result.castMeta.inputSnapshot?.date)
        lines.push(`- 起卦时间: ${result.castMeta.inputSnapshot.date}`);
    if (result.castMeta.inputSnapshot?.text)
        lines.push(`- 原始文本: ${result.castMeta.inputSnapshot.text}`);
    if (result.castMeta.inputSnapshot?.sentences?.length)
        lines.push(`- 分句: ${result.castMeta.inputSnapshot.sentences.join(' / ')}`);
    if (result.castMeta.inputSnapshot?.selectedText)
        lines.push(`- 取用文本: ${result.castMeta.inputSnapshot.selectedText}`);
    if (result.castMeta.inputSnapshot?.multiSentenceStrategy)
        lines.push(`- 取句方式: ${result.castMeta.inputSnapshot.multiSentenceStrategy === 'first' ? '首句' : '末句'}`);
    ...
    if (result.mainHexagram.guaCi)
        lines.push(`- 本卦卦辞: ${result.mainHexagram.guaCi}`);
    if (result.mainHexagram.xiangCi)
        lines.push(`- 本卦象辞: ${result.mainHexagram.xiangCi}`);
    ...
    if (result.changedHexagram) {
        ...
        if (result.changedHexagram.guaCi)
            lines.push(`- 变卦卦辞: ${result.changedHexagram.guaCi}`);
        if (result.changedHexagram.xiangCi)
            lines.push(`- 变卦象辞: ${result.changedHexagram.xiangCi}`);
    }
    ...
    if (detailLevel === 'full') {
        if (result.seasonalState.bodyMutual)
            lines.push(`- 体互旺衰: ${result.seasonalState.bodyMutual}`);
        if (result.seasonalState.useMutual)
            lines.push(`- 用互旺衰: ${result.seasonalState.useMutual}`);
        if (result.seasonalState.changed)
            lines.push(`- 变卦旺衰: ${result.seasonalState.changed}`);
    }
    lines.push('');
    lines.push('## 克应层次');
