...
function modToRange(value, base) {
    const result = value % base;
    return result === 0 ? base : result;
}
const CUE_LOOKUP = new Map();
...
function buildHexagramInfo(code) {
    ...
    return {
        name: hexagram.name,
        code: hexagram.code,
        upperTrigram: TRIGRAMS[hexagram.upperTrigram],
        lowerTrigram: TRIGRAMS[hexagram.lowerTrigram],
        element: hexagram.element,
        guaCi: GUA_CI[hexagram.name],
        xiangCi: XIANG_CI[hexagram.name],
    };
}
function buildChangedCode(code, movingLine) {
    const chars = code.split('');
    const index = movingLine - 1;
    chars[index] = chars[index] === '1' ? '0' : '1';
    return chars.join('');
}
function buildBasicNuclearCode(code) {
    return `${code.slice(1, 4)}${code.slice(2, 5)}`;
}
function buildNuclearCode(code, changedCode) {
...
function getRelation(body, other, stageLabel) {
    if (body === other) {
...
function buildInteraction(stage, stageLabel, body, other) {
...
function getSeasonState(monthBranch, element) {
...
function parseWallClockDateTime(date) {
    ...
    const isValid = utcDate.getUTCFullYear() === year &&
...
function normalizeMovingLine(value) {
    if (!Number.isInteger(value) || value < 1 || value > 6) {
        throw new Error('movingLine 必须是 1-6 的整数');
    }
    return value;
}
function normalizeCountCategory(value) {
...
function normalizeTextSplitMode(value) {
...
function normalizeMultiSentenceStrategy(value) {
    if (value === 'first' || value === 'last') {
        return value;
    }
    throw new Error('multiSentenceStrategy 仅支持 first/last');
}
function normalizeCueKey(value, fieldLabel = 'value') {
    return normalizeNonEmptyString(value, fieldLabel).replace(/\s+/g, '');
}
function normalizeSentencePairInput(value) {
...
function resolveCueToTrigram(value, fieldLabel, category) {
...
function mapModernPinyinToClassicalTone(value) {
...
function resolveClassicalToneValue(char, candidates) {
...
function getPinyinPronunciations(text, heteronym) {
...
function resolveClassicalToneNumbers(chars) {
    ...
    const toneValues = [];
    let ambiguousPronunciations = null;
...
function resolveAutoTextSplitNumbers(chars) {
    if (chars.length === 0) {
...
function castByCountWithTime(input, date) {
...
function castByTextSplit(input, date) {
...
function normalizeMeasureKind(kind) {
    if (kind === '丈尺')
        return '丈尺';
    if (kind === '尺寸')
        return '尺寸';
...
function castByMeasure(input, date) {
...
function castByClassifierPair(input, date) {
...
function castBySelect(input, date) {
    ...
    if (input.hexagramName) {
        ...
        upper = TRIGRAMS[hexagram.upperTrigram];
        lower = TRIGRAMS[hexagram.lowerTrigram];
    }
...
function castByExtendedNumbers(input, method, date) {
...
function resolveCast(input, date) {
    ...
    switch (method) {
        ...
        default:
            throw new Error(`不支持的梅花起卦方式: ${method}`);
    }
}
function buildTimingHints(readings) {
    const phaseMap = {
        use: 'early',
        body_mutual: 'middle',
        use_mutual: 'middle',
...
function buildJudgement(readings, seasonalState, bodyElement, useElement, bodyMutualElement, useMutualElement, changedElement) {
    let weight = 0;
    const basis = [];
    ...
    const trend = useReading && changedReading && useReading.favorable && !changedReading.favorable
        ? 'first_good_then_bad'
    ...
    const summary = trend === 'first_good_then_bad'
        ? '先吉后凶，初看有利，后势转阻。'
...
export function calculateMeihua(input) {
    ...
    const ganZhiTime = calculateGanZhiTime(analysisDate);
    ...
    const monthBranch = ganZhiTime.month.zhi;
    const seasonalState = {
        monthBranch,
    ...
    const judgement = buildJudgement(interactionReadings, seasonalState, bodyTrigram.element, useTrigram.element, bodyMutualTrigram.element, useMutualTrigram.element, changedHexagram.element);
    return {
        question,
        castMeta: cast.meta,
        ganZhiTime,
