...
import { calculateTenGod, TIAN_GAN, DI_ZHI, getStemYinYang } from '../utils.js';
import { getNaYin, getDiShi, buildHiddenStems } from './bazi.js';
import { calculateBranchShenSha } from '../shensha.js';
import { LIU_HE, SAN_HE, LIU_CHONG, XIANG_HAI, XIANG_XING, NA_YIN_TABLE, } from '../data/shensha-data.js';
const HIDDEN_STEM_MAIN = {
    '子': '癸', '丑': '己', '寅': '甲', '卯': '乙',
    '辰': '戊', '巳': '丙', '午': '丁', '未': '己',
    '申': '庚', '酉': '辛', '戌': '戊', '亥': '壬',
};
 ...
 * 计算一个地支与一组地支之间的所有关系
 */
function calculateBranchRelations(targetBranch, natalBranches, natalLabels) {
    const relations = [];
    // 六合
    ...
    for (const sanHe of SAN_HE) {
        if (!sanHe.branches.includes(targetBranch))
            continue;
    ...
    for (const xing of XIANG_XING) {
        if (!xing.combination.includes(targetBranch))
            continue;
 ...
 * 计算太岁关系（流年地支 vs 年柱地支）
 */
function calculateTaiSui(liunianBranch, yearBranch) {
    const result = [];
    // 值太岁
 ...
 * 计算小运
 */
function calculateXiaoYun(hourStem, hourBranch, gender, yearStem, dayStem, startAge) {
    if (startAge <= 1)
        return [];
...
function formatStartAgeDetail(birthYear, birthMonth, birthDay, birthHour, birthMinute, defaultStartAge, startSolar) {
    if (!startSolar) {
...
export function calculateDayunData(input) {
    ...
    const startAgeDetail = formatStartAgeDetail(birthYear, birthMonth, birthDay, birthHour, birthMinute, startAge, yun.getStartSolar());
    return { startAge, startAgeDetail, xiaoYun, list };
}
export async function handleDayunCalculate(input) {
