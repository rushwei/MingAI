...
export function renderZiweiCanonicalText(result, options = {}) {
    ...
    if (result.douJun)
        lines.push(`- **子年斗君**: ${result.douJun}`);
    if (result.lifeMasterStar)
        lines.push(`- **命主星**: ${result.lifeMasterStar}`);
    if (result.bodyMasterStar)
        lines.push(`- **身主星**: ${result.bodyMasterStar}`);
    if (result.trueSolarTimeInfo) {
        lines.push(...formatTrueSolarBlock(result.trueSolarTimeInfo));
    }
    lines.push('');
...
export function renderTarotCanonicalText(result, options = {}) {
    ...
    if (options.birthDate?.trim())
        lines.push(`- **出生日期**: ${options.birthDate.trim()}`);
    lines.push('');
...
export function renderQimenCanonicalText(result) {
...
export function renderDaliurenCanonicalText(result) {
    const lines = [
        '# 大六壬排盘',
        '',
        '## 基本信息',
    ...
    if (result.keTi.subTypes.length > 0)
        lines.push(`- **课体**: ${result.keTi.subTypes.join('、')}`);
    if (result.keTi.extraTypes.length > 0)
        lines.push(`- **附加**: ${result.keTi.extraTypes.join('、')}`);
    if (result.keName)
        lines.push(`- **课名**: ${result.keName}`);
    if (result.benMing)
        lines.push(`- **本命**: ${result.benMing}`);
    if (result.xingNian)
        lines.push(`- **行年**: ${result.xingNian}`);
    if (result.question)
        lines.push(`- **占事**: ${result.question}`);
    lines.push('');
    lines.push('## 四课');
    lines.push('');
...
export function renderFortuneCanonicalText(result) {
    ...
    if (almanac) {
        ...
        if (almanac.chongSha)
            lines.push(`- **冲煞**: ${almanac.chongSha}`);
        if (almanac.pengZuBaiJi)
            lines.push(`- **彭祖百忌**: ${almanac.pengZuBaiJi}`);
        if (almanac.taiShen)
            lines.push(`- **胎神占方**: ${almanac.taiShen}`);
        if (almanac.dayNineStar)
            lines.push(`- **日九星**: ${almanac.dayNineStar.description}（${almanac.dayNineStar.position}）`);
        if (almanac.suitable.length > 0) {
            lines.push('');
            lines.push('### 宜');
            for (const item of almanac.suitable)
                lines.push(`- ${item}`);
        }
        if (almanac.avoid.length > 0) {
            lines.push('');
            lines.push('### 忌');
            for (const item of almanac.avoid)
                lines.push(`- ${item}`);
        }
