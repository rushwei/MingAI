...
// 完整的78张塔罗牌
const FULL_DECK = [...MAJOR_ARCANA, ...WANDS, ...CUPS, ...SWORDS, ...PENTACLES];
// 牌阵定义
const SPREADS = {
...
// 获取牌义
function getMeaning(orientation, keywords, reversedKeywords) {
...
// 计算生命牌/人格牌和灵魂牌
function calculatePersonalityAndSoulCards(birthYear, birthMonth, birthDay) {
...
// 计算年度牌
function calculateYearlyCard(birthMonth, birthDay, currentYear) {
    const digits = `${birthMonth}${birthDay}${currentYear}`.split('').reduce((s, d) => s + Number(d), 0);
    return reduceToArcana(digits);
}
// 根据大阿卡纳编号获取牌名
function getMajorArcanaByNumber(num) {
    return MAJOR_ARCANA.find(c => c.number === num);
}
export async function calculateTarotData(input) {
