...
`;
  ...
  for (let i = 0; i < pillarKeys.length; i++) {
    ...
    if (pillar.hiddenStems && pillar.hiddenStems.length > 0) {
      const parts = pillar.hiddenStems.map(
        (hs) => `${hs.stem}(${hs.qiType}${hs.tenGod ? '·' + hs.tenGod : ''})`
      );
...
export function formatDaliurenAsMarkdown(result: DaliurenOutput): string {
  ...
  const formatGuiRen = (map?: Record<string, string>): string => {
    if (!map) return '-';
...
`;
  ...
  if (result.benMing || result.xingNian) {
    md += `## 本命行年\n`;
    if (result.benMing) md += `- **本命**: ${result.benMing}\n`;
    if (result.xingNian) md += `- **行年**: ${result.xingNian}\n`;
    md += '\n';
  }
