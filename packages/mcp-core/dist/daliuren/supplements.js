 ...
 * 计算某地支在日干五行下的十二长生状态
 */
export function getChangSheng(riGan, diZhi) {
...
export function getWangShuai(yueZhi, wuXing) {
...
export function getTaoHua(riZhi) {
    const idx = zhiIndex(riZhi);
    // 三合局：寅午戌(火)、申子辰(水)、巳酉丑(金)、亥卯未(木)
    const group = idx % 4; // 0:子辰申 1:丑巳酉 2:寅午戌 3:卯未亥
    // 按三合局分组
    if (['寅', '午', '戌'].includes(riZhi))
        return '卯';
    if (['申', '子', '辰'].includes(riZhi))
        return '酉';
    if (['巳', '酉', '丑'].includes(riZhi))
        return '午';
    if (['亥', '卯', '未'].includes(riZhi))
        return '子';
    return '';
}
/** 游都：传统算法较复杂且各家说法不一，暂不实现 */
export function getYouDu(_yueJiang) {
    return '';
}
/**
 * 判断课体细分
 ...
 * tianPan: 天盘
 */
export function classifyKeTi(method, siKe, sanChuan, tianPan, kongWang) {
    const subTypes = [];
    const extraTypes = [];
    // --- 取传课体子类型 ---
    if (method === '遥克') {
    ...
    // 空亡相关
    if (kongWang.includes(sanChuan.chu)) {
        // 初传逢空
        if (subTypes.includes('蒿失')) {
            // 蒿失逢空 — 已在蒿失中
        }
    }
...
/** 五行相克：a 克 b */
function keRelation(a, b) {
    const keMap = { 木: '土', 土: '水', 水: '火', 火: '金', 金: '木' };
    return keMap[a] === b;
}
/** 五行相生：a 生 b */
function shengRelation(a, b) {
 ...
 */
export function generateKeName(riGanZhi, ganYang) {
    // 局数从亥开始计数
 ...
 * 计算本命（出生年干支）和行年
 */
export function calcBenMingXingNian(birthYear, currentYear, gender) {
    // 本命 = 出生年干支
    const benMingGanIdx = (birthYear - 4) % 10;
    const benMingZhiIdx = (birthYear - 4) % 12;
...
// ===== 导出常量 =====
export { DI_ZHI, TIAN_GAN, ZHI_WUXING, GAN_WUXING, YUE_JIANG_NAMES, TIAN_JIANG_FULL, JI_GONG, zhiIndex, ganIndex, ganYinYang, liuChong, };
