...
import { DI_ZHI, ZHI_WUXING, YUE_JIANG_NAMES, TIAN_JIANG_FULL, getChangSheng, getWangShuai, getTaoHua, getYouDu, classifyKeTi, generateKeName, calcBenMingXingNian, } from './supplements.js';
/**
 * 大六壬排盘主函数
 */
export function handleDaliurenCalculate(input) {
    ...
    // 判断昼夜：卯时~酉时为昼，酉时~卯时为夜
    const diurnal = hour >= 6 && hour < 18;
    // 4. 构建 dateInfo
    const dateInfo = {
    ...
    // 补充游都
    const youDu = getYouDu(yueJiang);
    if (youDu && !shenSha.find(s => s.name === '游都')) {
        shenSha.push({ name: '游都', value: youDu, description: '主游移不定、出行。' });
    }
    // 11. 十二宫详情（含十二长生、五行旺衰）
    const gongInfos = DI_ZHI.map((diZhi) => {
        const key = String(DI_ZHI.indexOf(diZhi));
        ...
        return {
            ...
            tianZhi,
            tianJiang,
            tianJiangShort: TIAN_JIANG_FULL[tianJiang] || tianJiang,
            dunGan,
