...
// 天德贵人表（根据月支查天干）
const TIAN_DE_GUI_REN = {
    '寅': '丁', '卯': '申', '辰': '壬', '巳': '辛', '午': '亥', '未': '甲',
    '申': '癸', '酉': '寅', '戌': '丙', '亥': '乙', '子': '巳', '丑': '庚',
};
// 月德贵人表（根据月支查天干）
const YUE_DE_GUI_REN = {
    '寅': '丙', '午': '丙', '戌': '丙',
    '申': '壬', '子': '壬', '辰': '壬',
    '巳': '庚', '酉': '庚', '丑': '庚',
    '亥': '甲', '卯': '甲', '未': '甲',
...
// 计算神煞
function calculateShenSha(dayStem, dayBranch, yearBranch, monthBranch, hourBranch) {
    ...
    if (yiMaFound.length > 0)
        shenSha.yiMa = [...new Set(yiMaFound)];
    // 桃花（以年支和日支为准）
    ...
    if (taoHuaFound.length > 0)
        shenSha.taoHua = [...new Set(taoHuaFound)];
    // 华盖（以年支和日支为准）
    ...
    if (huaGaiFound.length > 0)
        shenSha.huaGai = [...new Set(huaGaiFound)];
    // 将星（以年支和日支为准）
    ...
    if (tianLuoDiWang.length > 0)
        shenSha.tianLuodiWang = tianLuoDiWang;
    return shenSha;
}
// 获取纳音
...
export async function handleBaziCalculate(input) {
    ...
    // 计算神煞
    const shenSha = calculateShenSha(dayStem, eightChar.getDayZhi(), eightChar.getYearZhi(), eightChar.getMonthZhi(), eightChar.getTimeZhi());
    return {
        gender,
        birthPlace,
        dayMaster: dayStem,
