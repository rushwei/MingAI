...
// 分析十神吉凶趋势
function analyzeTenGodTrend(tenGod) {
    const favorable = ['正财', '偏财', '正官', '正印', '食神'];
    const unfavorable = ['七杀', '伤官', '劫财'];
    if (favorable.includes(tenGod))
        return 'favorable';
    if (unfavorable.includes(tenGod))
        return 'unfavorable';
    return 'neutral';
}
// 生成关键因素分析
function generateKeyFactors(daYunTenGod, liunianTenGod, liuyueTenGod) {
    const factors = [];
    // 大运分析
...
export async function handleLiunianAnalyze(input) {
    const { gender, birthYear, birthMonth, birthDay, birthHour, birthMinute = 0, calendarType = 'solar', isLeapMonth = false, targetYear, targetMonth, } = input;
    // 创建 Solar 对象
    ...
    if (!currentDaYun.ganZhi && daYunList.length > 0) {
        ...
        const startYear = lastDy.getStartYear();
    ...
    // 生成关键因素
    const keyFactors = generateKeyFactors(currentDaYun.tenGod, liunianTenGod, liuyue?.tenGod);
    return {
        currentDaYun,
        liunian: {
            year: currentYear,
