...
const packageJsonPath = resolve(process.cwd(), 'packages/mcp-core/package.json');
const toolsPath = resolve(process.cwd(), 'packages/mcp-core/src/tools.ts');
const toolSchemaPath = resolve(process.cwd(), 'packages/mcp-core/src/tool-schema.ts');
const indexPath = resolve(process.cwd(), 'packages/mcp-core/src/index.ts');
const formattersPath = resolve(process.cwd(), 'packages/mcp-core/src/formatters.ts');

test('qimen runtime dependency should be declared in mcp-core package manifest', async () => {
  const pkg = JSON.parse(await readFile(packageJsonPath, 'utf-8'));

  assert.equal(typeof pkg.dependencies?.taobi, 'string', 'mcp-core should declare taobi as a runtime dependency');
});

test('mcp-core tool surface should expose qimen寄宫 option and avoid re-adding removed legacy aliases', async () => {
