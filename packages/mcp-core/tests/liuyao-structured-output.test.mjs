...
test('liuyao output uses refactored yao/yongshen/time structures', async () => {
  ...
  for (const group of result.yongShen) {
    ...
    const candidates = [group.selected, ...group.candidates];
    for (let i = 1; i < candidates.length; i++) {
      assert.ok(candidates[i - 1].rankScore >= candidates[i].rankScore, 'candidates should be sorted by rankScore desc');
    }
  }
