...
type YaoMovementState = 'static' | 'changing' | 'hidden_moving' | 'day_break';

// 旺衰标签
const WANG_SHUAI_LABELS: Record<WangShuai, string> = {
  wang: '旺', xiang: '相', xiu: '休', qiu: '囚', si: '死',
};

// 空亡状态标签
const KONG_WANG_LABELS: Record<KongWangState, string> = {
  'not_kong': '',
  'kong_static': '空',
  'kong_changing': '动空',
  'kong_ri_chong': '冲空',
...
const MOVEMENT_LABELS: Record<YaoMovementState, string> = {
  static: '静',
  changing: '明动',
  hidden_moving: '暗动',
  day_break: '日破',
...
// 象辞
const XIANG_CI: Record<string, string> = {
  '乾为天': '天行健，君子以自强不息。',
  '坤为地': '地势坤，君子以厚德载物。',
  '水雷屯': '云雷，屯；君子以经纶。',
  '山水蒙': '山下出泉，蒙；君子以果行育德。',
...
// 卦辞简介
const HEXAGRAM_BRIEF: Record<string, string> = {
  '乾为天': '元亨利贞，君子自强不息。',
  '坤为地': '厚德载物，柔顺利贞。',
  '水雷屯': '万事开头难，需耐心等待时机。',
  '山水蒙': '蒙昧待启，虚心求教。',
...
// 五行长生起点（阳干顺行，阴干逆行）
const CHANG_SHENG_START: Record<WuXing, DiZhi> = {
  '木': '亥', '火': '寅', '土': '寅', '金': '巳', '水': '申',
};

// 计算十二长生
function getChangSheng(yaoElement: WuXing, naJia: DiZhi): string {
...
// 变爻分析类型
type HuaType = 'hua_jin' | 'hua_tui' | 'hua_sheng' | 'hua_ke' | 'hua_kong' | 'hua_jue' | 'hua_mu' | 'fu_yin';

// 变爻分析标签
const HUA_TYPE_LABELS: Record<HuaType, string> = {
  'hua_jin': '化进',
  'hua_tui': '化退',
  'hua_sheng': '化生',
  'hua_ke': '化克',
...
function analyzeYaoChange(
  originalElement: WuXing,
  changedElement: WuXing,
  originalNaJia: DiZhi,
  changedNaJia: DiZhi,
  kongWang: KongWang
): { huaType: HuaType; huaLabel: string; isGood: boolean } {
  ...
  // 检查化墓
  const muZhi: Record<WuXing, DiZhi> = { '木': '未', '火': '戌', '土': '戌', '金': '丑', '水': '辰' };
  if (changedNaJia === muZhi[originalElement]) {
    return { huaType: 'hua_mu', huaLabel: HUA_TYPE_LABELS['hua_mu'], isGood: false };
  }

  // 检查化绝
  const jueZhi: Record<WuXing, DiZhi> = { '木': '申', '火': '亥', '土': '亥', '金': '寅', '水': '巳' };
...
function checkYaoKongWang(
  ...
  if (!isKong) return 'not_kong';

  // 日冲空亡
  if (LIU_CHONG[naJia] === dayZhi) return 'kong_ri_chong';
  // 月建临空
  if (naJia === monthZhi) return 'kong_yue_jian';
  // 动空
  if (isChanging) return 'kong_changing';
  // 静空
  return 'kong_static';
}

// 分析三合局
function analyzeSanHe(yaos: InternalYaoInfo[], monthZhi: DiZhi, dayZhi: DiZhi): {
  hasFullSanHe: boolean;
  fullSanHe?: { name: string; result: WuXing; positions: number[] };
  hasBanHe: boolean;
  banHe?: Array<{ branches: string[]; result: WuXing; type: string; positions: number[] }>;
} {
...
// 检测六冲卦
function checkLiuChongGua(yaos: InternalYaoInfo[]): { isLiuChongGua: boolean; description?: string } {
...
function calculateShenSystem(
  ...
  const findPositions = (element: WuXing) =>
    yaos.filter(y => y.wuXing === element).map(y => y.position);
...
function calculateTimeWindowByBranch(
  ...
  const targetIdx = DIZHI_LIST.indexOf(targetBranch as DiZhi);
...
function generateWarnings(
  yongShen: { liuQin: LiuQin; element: WuXing; position: number; isStrong: boolean; kongWangState?: KongWangState },
  shenSystem: ReturnType<typeof calculateShenSystem>,
  sanHeAnalysis: ReturnType<typeof analyzeSanHe>,
  yaos: InternalYaoInfo[]
): string[] {
...
function calculateFuShen(
  ...
  for (const targetLiuQin of missingLiuQin) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const naJiaIdx = i % 3;
      const naJia = (gong?.naJia[isLower ? 0 : 1][naJiaIdx] || '子') as DiZhi;
...
const BA_GONG_GUI_SHU: Record<string, { gong: string; order: number }> = {
  // 乾宫
...
// 八宫归属表（续）
const BA_GONG_GUI_SHU_2: Record<string, { gong: string; order: number }> = {
  // 巽宫
...
// 八宫纳甲表
const BA_GONG_NA_JIA: Record<string, { element: WuXing; naJia: DiZhi[][] }> = {
...
// 根据卦名或卦码查找卦象
function findHexagram(input: string): Hexagram | undefined {
  if (/^[01]{6}$/.test(input)) {
...
// 获取干支时间
function getGanZhiTime(date: Date): GanZhiTime {
...
// 计算爻的旺衰
function getYaoStrength(yaoElement: WuXing, monthZhi: DiZhi): { wangShuai: WangShuai; isStrong: boolean } {
...
// 根据问题类型推断用神（支持多目标）
function inferYongShenTargets(question: string): { type: string; targets: LiuQin[] } {
...
export async function handleLiuyaoAnalyze(input: LiuyaoInput): Promise<LiuyaoOutput> {
  ...
  let mainHexagramName: string;
  let finalChangedHexagramName: string | undefined;
  ...
  const shenShaContext = {
    ...
    kongWang: { xun: kongWang.xun, kongZhi: [kongWang.kongZhi[0], kongWang.kongZhi[1]] as [string, string] },
  };

  const changedYaoByPosition = new Map<number, {
    type: number;
    liuQin: string;
    naJia: string;
    wuXing: string;
  ...
  if (changedCode && changedLines.length > 0) {
    ...
    if (changedHexInfo) {
      ...
      for (const pos of changedLines) {
        ...
        const wuXing = DIZHI_WUXING[naJia];
        const liuQin = getLiuQin(gongElement, wuXing);
        const changedType = parseInt(changedCode[idx], 10) as YaoType;
  ...
  const targetSpecs = normalizedTargets.length > 0
    ? normalizedTargets.map(target => ({ target, source: 'input' as const }))
    : inferredTargets.targets.map(target => ({ target, source: 'inferred' as const }));
  ...
  const yongShen = targetSpecs.map((spec) => {
    const rankedCandidates = fullYaos
      ...
      .map((y) => {
        ...
        if (strength.isStrong) factors.push('月令生扶');
        if (movementState !== 'static') factors.push(MOVEMENT_LABELS[movementState]);
        if (kongState !== 'not_kong') factors.push(KONG_WANG_LABELS[kongState]);
    ...
    const selected = rankedCandidates[0] ?? {
      ...
      isShiYao: false,
      isYingYao: false,
      kongWangState: 'not_kong' as KongWangState,
      rankScore: 0,
      factors: ['目标六亲不上卦'],
    };
  ...
  const shenSystemByYongShen = yongShen.map((group) => {
    const selectedElement = (group.selected.element || '土') as WuXing;
  ...
  for (const group of yongShen) {
    if (!group.selected.isStrong) warnings.push(`用神${group.targetLiuQin}力弱`);
    if (group.selected.kongWangState === 'kong_static') warnings.push(`用神${group.targetLiuQin}空亡`);
  }
  ...
  return {
    ...
    fullYaos: fullYaos.map((y) => {
      ...
      if (yaoStrength.isStrong) strengthFactors.push('月令生扶');
      if (movementState !== 'static') strengthFactors.push(MOVEMENT_LABELS[movementState]);
      if (yaoKongWang !== 'not_kong') strengthFactors.push(KONG_WANG_LABELS[yaoKongWang]);
