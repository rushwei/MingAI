...
// 根据十神计算运势分数
function calculateScores(tenGod: string, rng: () => number): {
  overall: number;
  career: number;
  love: number;
  wealth: number;
...
// 获取幸运颜色（基于日主五行的相生元素）
function getLuckyColor(dayMaster: string, rng: () => number): string {
  const element = STEM_ELEMENTS[dayMaster];
  if (!element) return '白色';
...
// 获取幸运方位（基于日主五行的相生元素）
function getLuckyDirection(dayMaster: string): string {
  const element = STEM_ELEMENTS[dayMaster];
  if (!element) return '东方';
...
// 生成建议
function generateAdvice(tenGod: string): string[] {
...
export async function handleDailyFortune(input: FortuneInput): Promise<FortuneOutput> {
  ...
  const seed = resolveSeed(
    input.seed,
    `${dateKey}|${input.dayMaster || ''}|${input.birthYear || ''}-${input.birthMonth || ''}-${input.birthDay || ''}-${input.birthHour || ''}`,
    input.seedScope
  );
