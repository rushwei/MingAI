...
function isPlaceResolutionTool(toolName) {
...
function isAmapPrecisionSufficient(level) {
...
function buildFallbackInfo(requestedPlace, reason) {
    return {
        requestedPlace,
        resolved: false,
        source: 'fallback',
...
function withDescriptionSuffix(text, suffix) {
    return text ? `${text} ${suffix}` : suffix;
}
function appendPlaceResolutionNote(markdown, info) {
    const lines = ['## 出生地解析'];
    ...
    if (info.source === 'manual_longitude') {
        ...
        if (info.level)
            lines.push(`- **解析级别**: ${info.level}`);
        if (info.usedLongitude != null)
            lines.push(`- **使用经度**: ${info.usedLongitude}°`);
        lines.push('- **真太阳时**: 已启用');
    }
    else {
...
export async function preprocessToolArgsForRuntimePlace(toolName, args) {
...
export function decorateToolListPayloadForRuntime(payload) {
    return {
        ...payload,
        tools: payload.tools.map((tool) => {
            ...
            if (inputProperties.longitude && typeof inputProperties.longitude === 'object' && inputProperties.longitude !== null) {
                inputProperties.longitude = {
                    ...inputProperties.longitude,
                    description: withDescriptionSuffix(String(inputProperties.longitude.description || ''), '若同时提供 longitude 和 birthPlace，优先使用 longitude；若只有地点名，在线 MCP Server 会先做地理编码。'),
                };
            }
            ...
            return {
                ...tool,
                description: withDescriptionSuffix(tool.description, '在线 MCP Server 支持仅传 birthPlace；若未显式提供 longitude，会尝试做地理编码并在失败时自动退化。'),
...
export function attachPlaceResolutionInfoToResult(result, placeResolutionInfo) {
...
export function attachPlaceResolutionNoteToPayload(payload, placeResolutionInfo, responseFormat) {
