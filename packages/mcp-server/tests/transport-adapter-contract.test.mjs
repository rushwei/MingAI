...
test('mcp server and local entrypoints should share a transport adapter from mcp-core', async () => {
  const serverSource = await readFile(resolve(process.cwd(), 'packages/mcp-server/src/index.ts'), 'utf-8');
  const localSource = await readFile(resolve(process.cwd(), 'packages/mcp-local/src/index.ts'), 'utf-8');
