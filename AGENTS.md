...
## Project Background
MingAI is an AI-driven Chinese astrology platform spanning 八字, 紫微斗数, 关系合盘, MBTI, 塔罗, 六爻, 运势中心, 命理社区, 面相/手相 with AI chat, daily/monthly fortunes, gamification, reminders, subscription tiers, annual reports, and site/email notifications. The product emphasizes a calm, modern UI and privacy-first data handling. Feature status and scope live in `docs/plans/PRD-MingAI-v2.0.md` and delivery reports under `docs/deliverables/`.

## Architecture & Stack
Next.js 16 App Router + React 19 + TypeScript; Capacitor wraps the web app for iOS/Android. UI uses Tailwind CSS and Lucide icons. Supabase provides Auth + Postgres + Storage; Resend is used for email notifications. AI inference uses DeepSeek/GLM/Gemini/Qwen (plus Qwen-VL/Gemini-VL for vision). Deployments go to Vercel + Zeabur. Cron-like tasks run via GitHub Actions (credits restore, reminders, annual reports) calling `/api` routes.

## Project Structure & Module Organization
- `src/app`: App Router routes and API handlers (`src/app/api`), including `community`, `records`, `fortune-hub`, `face`, `palm`.
- `src/components`: feature components (`admin`, `auth`, `bazi`, `ziwei`, `chat`, `daily`, `fortune`, `hepan`, `liuyao`, `mbti`, `tarot`, `membership`, `notification`, `profile`) and shared UI/layout (`layout`, `ui`, `common`, `providers`).
- `src/lib`: domain logic (AI/providers, astrology systems, community, records, gamification, reminders, membership, notifications, credits).
...
Key tables: `users` (membership + `ai_chat_count` + `is_admin`), `user_settings`, `bazi_charts`, `ziwei_charts`, `hepan_charts`, `mbti_readings`, `tarot_readings`, `liuyao_divinations`, `conversations`, `notifications`, `feature_subscriptions`, `orders`, `credit_transactions`, `daily_checkins`, `user_levels`, `user_achievements`, `community_posts`, `community_comments`, `community_votes`, `community_reports`, `community_anonymous_mapping`, `ming_records`, `ming_notes`, `face_readings`, `palm_readings`, `reminder_subscriptions`, `scheduled_reminders`, `annual_reports`, `app_settings`, `login_attempts`, `rate_limits`. RLS is enabled; `notifications` inserts are service-role only. Admin operations use service-side Supabase clients; do not expose service keys to the client.
