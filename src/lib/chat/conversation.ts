...
export async function loadConversations(userId: string): Promise<Conversation[]> {
    ...
    return (data || []).map((row: Record<string, unknown>) => ({
        ...
        personality: normalizePersonality(row.personality as string | null),
        title: row.title,
        messages: hydrateConversationMessages((row.messages as ChatMessage[]) || [], row.source_data as Record<string, unknown> | undefined),
