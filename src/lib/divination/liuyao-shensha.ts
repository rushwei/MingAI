...
export function calculateBranchShenSha(
  ...
  matchValue((TIAN_YI_GUI_REN as Record<string, string[]>)[dayStem], targetBranch, '天乙贵人', names);
  matchValue((TAI_JI_GUI_REN as Record<string, string[]>)[dayStem], targetBranch, '太极贵人', names);
  ...
  if ((GU_CHEN as Record<string, string>)[yearBranch] === targetBranch) addUnique(names, '孤辰');
  if ((GUA_SU as Record<string, string>)[yearBranch] === targetBranch) addUnique(names, '寡宿');
  if ((JIANG_XING as Record<string, string>)[yearBranch] === targetBranch) addUnique(names, '将星');
